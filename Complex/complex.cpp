...
Complex::Complex(double my_real, double my_img):real(my_real),
    img(my_img){
}
...
Complex& Complex::operator /= (const Complex &c){
    real = (real * c.real + img * c.img)/(c.img * c.img + c.real * c.real);
    img = (img * c.real - real * c.img)/(c.img * c.img + c.real * c.real);
    return *this;
}
